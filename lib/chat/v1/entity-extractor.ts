...
export function extractEntities(message: string): ExtractedEntities {
  ...
  if (amountMatch) {
    ...
    if (fallbackMatch) {
      ...
      if (parsed > 0 && (normalized.includes("spent") || normalized.includes("add") || normalized.includes("paid") || normalized.includes("budget") || normalized.includes("salary"))) {
  ...
  for (const token of tokens) {
    ...
    if (cleanToken === "spent" || cleanToken === "paid" || cleanToken === "bought") {
      continue;
    }
  ...
  if (normalized.includes("today")) {
    ...
    entities.fromDate = new Date(today.getFullYear(), today.getMonth(), today.getDate());
    entities.toDate = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 23, 59, 59);
  } else if (normalized.includes("yesterday")) {
    entities.dateStr = "yesterday";
    const yesterday = new Date(today.getTime() - 24 * 60 * 60 * 1000);
    entities.fromDate = new Date(yesterday.getFullYear(), yesterday.getMonth(), yesterday.getDate());
    entities.toDate = new Date(yesterday.getFullYear(), yesterday.getMonth(), yesterday.getDate(), 23, 59, 59);
    ...
    entities.toDate = new Date(today.getFullYear(), today.getMonth() + 1, 0, 23, 59, 59);
    ...
    entities.toDate = new Date(today.getFullYear(), today.getMonth(), 0, 23, 59, 59);
  }

  // 4. Note extraction (heuristically)
  // "Add ₹250 for taxi today" -> taxi
  // "Record ₹1200 for groceries" -> groceries
  const noteMatch = normalized.match(/(?:for|on|worth)\s+([a-zA-Z\s]+?)(?:\s+(?:today|yesterday|tomorrow|this month|last month|₹|\d|$))/i);
  if (noteMatch) {

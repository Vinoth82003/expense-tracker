    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-20 px-4 sm:px-6">
          ...
          >
            <div className="flex items-center justify-between p-6 sm:p-8 bg-surface border border-border-subtle rounded-[2.5rem] print:hidden">
                ...
                <div className="flex items-center gap-3 print:hidden">
                    <button 
                        onClick={handlePrint}
                        className="p-3 rounded-xl bg-surface border border-border-subtle text-secondary hover:text-foreground transition-all shadow-sm active:scale-95 flex items-center gap-2 font-bold"
                    >
                        <Printer size={20} />
                        Print / Export
                    </button>
            ...
            <div className="hidden print:block mb-12 border-b-4 border-indigo-600 pb-6">
                <div className="flex justify-between items-end">
                    <div>
                        <h1 className="text-4xl font-black text-indigo-950 uppercase tracking-tighter">Financial Analysis Report</h1>
                        <p className="text-xl font-bold text-indigo-800/60 mt-2">SpendWise Premium AI Intelligence</p>
                    </div>
                    <div className="text-right">
                        <p className="text-sm font-black text-indigo-950">NAME: {session?.user?.name || "Premium User"}</p>
                        <p className="text-sm font-bold text-indigo-900/60 uppercase tracking-widest">DATE: {new Date().toLocaleDateString('en-IN')}</p>
                    </div>
                </div>
            </div>

            {/* Summary Card */}
            <div className="bg-surface border border-border-subtle rounded-[2.5rem] p-8 sm:p-12 shadow-sm print:shadow-none print:border-indigo-100 print:rounded-3xl print:p-0 print:mb-10">
                <h3 className="text-2xl font-black mb-6 flex items-center gap-2 print:text-xl print:text-indigo-950">
                    <Sparkles className="text-primary-500 print:text-indigo-600" />
                    Forensic Summary
                </h3>
                <div className="prose prose-indigo max-w-none dark:prose-invert prose-headings:text-foreground prose-headings:font-black prose-p:text-secondary prose-p:font-medium prose-strong:text-foreground prose-li:text-secondary print:prose-p:text-indigo-950/80">
                    <ThemedMarkdown content={report.summary} />
                </div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 print:grid-cols-1 print:gap-10">
                {/* Warnings Section */}
                <div className="bg-surface border border-border-subtle rounded-[2.5rem] p-8 shadow-sm group hover:border-error/30 transition-colors print:border-indigo-100 print:rounded-3xl print:p-0">
                    <h3 className="text-xl font-black text-foreground mb-6 flex items-center gap-2 print:text-indigo-950">
                        <AlertCircle size={24} className="text-error" />
                        Red Flags & Warnings
                    </h3>
                    <ul className="space-y-4">
                        {report.warnings.map((warning, i) => (
                            <li key={i} className="flex gap-3 text-secondary font-bold text-sm bg-surface-variant/30 p-4 rounded-2xl border border-border-subtle print:bg-indigo-50/30 print:border-indigo-100 print:text-indigo-950/80">
                ...
                {/* Suggestions Section */}
                <div className="bg-surface border border-border-subtle rounded-[2.5rem] p-8 shadow-sm group hover:border-primary-500/30 transition-colors print:border-indigo-100 print:rounded-3xl print:p-0">
                    <h3 className="text-xl font-black text-foreground mb-6 flex items-center gap-2 print:text-indigo-950">
            ...
            {/* Savings Plan */}
            <div className="bg-gradient-to-br from-indigo-500/5 to-violet-500/5 border border-border-subtle rounded-[3rem] p-8 sm:p-12 relative overflow-hidden group print:bg-none print:border-indigo-100 print:rounded-3xl print:p-0 print:mt-10">
                <div className="absolute right-0 top-0 p-8 text-primary-500/5 group-hover:scale-110 transition-transform print:hidden">
                    <TrendingUp size={120} />
                </div>
                <h3 className="text-2xl font-black mb-6 flex items-center gap-2 relative z-10 print:text-indigo-950">
                    <Star className="text-primary-500" />
                    Personalized Savings Plan
                </h3>
                <div className="prose prose-indigo max-w-none dark:prose-invert prose-headings:text-foreground prose-headings:font-black prose-p:text-secondary prose-p:font-bold prose-strong:text-foreground prose-li:text-secondary relative z-10 print:prose-p:text-indigo-950/80">
      ...
      <style jsx global>{`
        @media print {
          ...
          aside, nav, header, .print-hidden, button {
            display: none !important;
          }
          /* Ensure report container fills page */
          main, div {
            padding: 0 !important;
            margin: 0 !important;
          }
          /* Card optimizations for paper */
          .bg-surface, .bg-surface-variant, .bg-gradient-soft {
            background: white !important;
          ...
          .rounded-[2.5rem], .rounded-[3rem] { border-radius: 0.75rem !important; }
          .p-8, .p-12 { padding: 1.5rem 0 !important; }

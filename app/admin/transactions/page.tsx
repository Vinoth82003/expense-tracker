...
export default function AdminTransactionsPage() {
  ...
  return (
    <div className="space-y-8 pb-20">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">All transactions</h1>
        <p className="text-slate-500 dark:text-slate-400 font-medium">Platform-wide expense and income records</p>
      </div>
      ...
      <div className="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-2xl w-fit">
        <button 
          ...
          className={`px-8 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === "expenses" ? "bg-white dark:bg-slate-700 text-teal-600 dark:text-teal-400 shadow-sm" : "text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"}`}
        >
          Expenses
        </button>
        <button 
          onClick={() => { setActiveTab("income"); setPage(1); }}
          className={`px-8 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === "income" ? "bg-white dark:bg-slate-700 text-emerald-600 dark:text-emerald-400 shadow-sm" : "text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"}`}
        >
          Income
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {activeTab === "expenses" ? (
          <>
            ...
            <StatChip label="Records" value={stats?.recordCount?.toLocaleString() || "0"} color="text-slate-900 dark:text-white" />
            <StatChip label="Flagged" value={stats?.flaggedCount?.toString() || "0"} color="text-amber-500" />
            <StatChip label="Avg / User" value="₹10,840" color="text-slate-900 dark:text-white" />
          </>
        ) : (
          <>
            <StatChip label="Total Logged" value={`₹${(stats?.totalAmount || 0).toLocaleString()}`} color="text-emerald-500" />
            <StatChip label="Records" value={stats?.recordCount?.toLocaleString() || "0"} color="text-slate-900 dark:text-white" />
            <StatChip label="Avg / User" value="₹43,400" color="text-slate-900 dark:text-white" />
      ...
      {/* Filters Bar */}
      <div className="flex flex-col gap-4 bg-white dark:bg-[#161B27] p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex-1 min-w-[200px] relative">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-[#1E2536] border-none rounded-xl outline-none focus:ring-2 focus:ring-teal-500 transition-all text-sm font-medium"
            />
          </div>
          ...
          <select 
            ...
            className="px-4 py-2.5 bg-slate-50 dark:bg-[#1E2536] rounded-xl text-sm font-bold outline-none cursor-pointer border-none"
          >
            <option value="All">All Categories</option>
            {categories.map(c => <option key={c} value={c}>{c}</option>)}
          </select>

          <div className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 dark:bg-[#1E2536] rounded-xl border-none">
            <Calendar size={16} className="text-slate-400" />
            <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="bg-transparent border-none outline-none text-xs font-bold w-28" />
            <span className="text-slate-400 font-bold">-</span>
            <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="bg-transparent border-none outline-none text-xs font-bold w-28" />
          </div>

          <div className="flex items-center gap-2">
            <span className="text-[10px] font-black uppercase text-slate-400">Flagged</span>
            <button 
              onClick={() => setFlaggedOnly(!flaggedOnly)}
              className={`w-10 h-6 rounded-full relative transition-colors ${flaggedOnly ? 'bg-red-500' : 'bg-slate-200 dark:bg-slate-700'}`}
            >
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
              {data.map((tx) => (
                <tr 
                  ...
                  className={`group transition-colors cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 ${tx.isFlagged ? 'bg-amber-500/5 dark:bg-amber-500/10' : ''}`}
                >
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold text-slate-900 dark:text-slate-200 truncate max-w-[120px]">{tx.user.name || "Anonymous"}</span>
                    </div>
                  </td>
                  <td className="py-4 px-6">
                  ...
                  <td className="py-4 px-6">
                    <span className="px-2 py-1 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[10px] font-bold rounded-lg truncate max-w-[100px]">
                      {tx.subcategory || "-"}
                    </span>
                  </td>
                  <td className="py-4 px-6">
                  ...
                  <td className="py-4 px-6 text-xs font-bold text-slate-400">{new Date(tx.date).toLocaleDateString()}</td>
                  <td className="py-4 px-6">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium text-slate-500">Showing {(page-1)*25 + 1} to {Math.min(page*25, total)} of {total} transactions</p>
        <div className="flex items-center gap-2">
          <button 
            disabled={page === 1}
      ...
      <div className="bg-white dark:bg-[#161B27] rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <button 
          ...
          className="w-full flex items-center justify-between px-8 py-6 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"
        >
          <div className="flex items-center gap-3">
            <BarChart3 className="text-teal-500" />
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">Category Usage Analytics</h2>
          </div>
            ...
            >
              <div className="flex justify-end mb-6">
                <div className="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-xl w-fit">
                  <button onClick={() => setChartMode("volume")} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${chartMode === 'volume' ? 'bg-white dark:bg-slate-700 text-teal-600 shadow-sm' : 'text-slate-500'}`}>Volume (₹)</button>
                  <button onClick={() => setChartMode("count")} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${chartMode === 'count' ? 'bg-white dark:bg-slate-700 text-teal-600 shadow-sm' : 'text-slate-500'}`}>Count</button>
                </div>
              </div>
                        ...
                        if (active && payload && payload.length) {
                          return (
                            <div className="bg-white dark:bg-[#1E2536] p-3 rounded-xl border border-slate-100 dark:border-slate-800 shadow-xl">
                              <p className="text-xs font-bold text-slate-900 dark:text-white mb-1">{payload[0].payload.name}</p>
        ...
        {isDrawerOpen && selectedTx && (
          <div className="fixed inset-0 z-[100] flex justify-end">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-lg bg-white dark:bg-[#161B27] h-full shadow-2xl flex flex-col"
            >
              <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800">
                <h2 className="text-xl font-bold">Transaction Detail</h2>
                <button onClick={() => setIsDrawerOpen(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8">
                {/* User Info */}
                <div className="flex items-center gap-4 p-6 bg-slate-50 dark:bg-[#1E2536] rounded-3xl border border-slate-100 dark:border-slate-800">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white">{selectedTx.user.name || "Anonymous"}</h3>
                    <p className="text-sm font-medium text-slate-500">{selectedTx.user.email}</p>
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-center">
                    <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Transaction Data</p>
                  ...
                  <div className="p-6 bg-slate-50 dark:bg-[#1E2536] rounded-2xl border border-slate-100 dark:border-slate-800">
                    <p className="text-[10px] font-black uppercase text-slate-400 mb-2">Note</p>
                  ...
                  <div className="space-y-4">
                    <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Administrative Context</p>
                ...
                <div className="space-y-4 pt-8 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <textarea 
                    ...
                    className="w-full p-4 bg-slate-50 dark:bg-[#1E2536] border border-slate-100 dark:border-slate-800 rounded-2xl outline-none focus:ring-2 focus:ring-red-500 text-sm font-medium transition-all"
                  />
                  <button 
...
function StatChip({ label, value, color }: { label: string, value: string, color: string }) {
  return (
    <div className="p-6 bg-white dark:bg-[#161B27] rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-1">
...
function DetailBox({ label, value, valueColor }: { label: string, value: string, valueColor?: string }) {
  return (
    <div className="p-4 bg-slate-50 dark:bg-[#1E2536] rounded-2xl border border-slate-100 dark:border-slate-800">
      ...
      <p className={`text-sm font-bold ${valueColor || 'text-slate-900 dark:text-white'}`}>{value}</p>
    </div>
  );
}

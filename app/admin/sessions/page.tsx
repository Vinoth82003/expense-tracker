  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div>
        ...
        <p className="text-[var(--admin-text-secondary)] font-medium">Active sessions, OTP history, login audit</p>
      </div>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-4">
                <span className="text-sm font-bold text-[var(--admin-text-muted)] uppercase tracking-widest">{sessions.length} active sessions</span>
                <button 
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="pl-10 pr-4 py-2 bg-[var(--admin-bg-card)] border border-[var(--admin-border)] rounded-xl outline-none focus:ring-2 focus:ring-teal-500 transition-all text-sm font-medium"
                />
              </div>
            </div>
                  ...
                  <tbody className="divide-y divide-[var(--admin-border-subtle)]">
                    {sessions.map((session) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-1 text-xs font-medium text-[var(--admin-text-secondary)]">
                            <MapPin size={12} className="text-[var(--admin-text-muted)]" />
                            {session.location || "Unknown"}
                          </div>
                        </td>
                        <td className="py-4 px-6 text-xs font-bold text-[var(--admin-text-muted)]">{new Date(session.createdAt).toLocaleDateString()}</td>
                        <td className="py-4 px-6 text-xs font-bold text-[var(--admin-text-muted)]">{new Date(session.expires).toLocaleDateString()}</td>
                        <td className="py-4 px-6 text-right">
                          <button 
                            onClick={() => revokeSession(session.id)}
                            className="p-2 text-[var(--admin-text-muted)] hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-all"
                          >
                            <LogOut size={16} />
                          </button>
                        </td>
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <input 
                    ...
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                    className="pl-10 pr-4 py-2 bg-[var(--admin-bg-card)] border border-[var(--admin-border)] rounded-xl outline-none focus:ring-2 focus:ring-teal-500 transition-all text-sm font-medium text-[var(--admin-text-primary)]"
                  />
                </div>
                <select 
                  value={statusFilter}
                  onChange={(e) => setStatusFilter(e.target.value)}
                  className="px-4 py-2 bg-[var(--admin-bg-card)] border border-[var(--admin-border)] rounded-xl text-sm font-bold outline-none cursor-pointer text-[var(--admin-text-primary)]"
                >
                  ...
                  <tbody className="divide-y divide-[var(--admin-border-subtle)]">
                    {otpLogs.map((log) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-6 text-xs font-medium text-[var(--admin-text-secondary)]">{log.email}</td>
                        ...
                        <td className="py-4 px-6 text-right text-xs font-bold text-[var(--admin-text-secondary)]">
                          {log.attempts}
                        </td>
                      </tr>
                    ))}
